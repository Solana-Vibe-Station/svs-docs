...
add_solana(
    ...
    summary="Largest accounts of a token (default 20, up to 100)",
    description="Returns the largest accounts of a particular SPL Token type — the Solana-standard 20 by default, or up to 100 via the SVS-specific `limit` config field. This method is essential for analyzing token distribution, identifying whale accounts, and understanding token concentration patterns.",
    params=[pubkey_param(name="mint", desc="Token mint pubkey, base-58."), config_param(extra_props={
        "limit": s_int(
            desc="SVS extension (not part of the standard Solana RPC API): number of largest accounts to return, 1–100. Omit for the Solana-standard 20 — existing clients are unaffected. The full holder set is already sorted server-side, so a larger limit costs nothing extra. Values over the maximum (or otherwise invalid) are rejected with a `-32602` error naming the cap (e.g. `limit 101 exceeds the maximum of 100`) rather than silently clamped.",
...
def main(out_dir):
    ...
    for g in groups:
        ...
        for m in g["methods"]:
            ...
            # utf-8 + LF explicitly: Windows otherwise writes cp1252/CRLF,
            # which crashes on non-ASCII descriptions and dirties every file.
